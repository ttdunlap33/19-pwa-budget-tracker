...
function checkDatabase() {
  ...
  // Open a transaction on your newTransaction db
  let transaction = db.transaction(['newTransaction'], 'readwrite')

  // access your newTransaction object
    ...
    if (getAll.result.length > 0) {
      fetch('/api/transaction/bulk', {
        ...
        .then((res) => {
          ...
          if (res.length !== 0) {
            // Open another transaction to newTransaction with the ability to read and write
...
const saveRecord = (record) => {
  ...
  // Create a transaction on the newTransaction db with readwrite access
  const transaction = db.transaction(['newTransaction'], 'readwrite')

  // Access your newTransaction object store
  const store = transaction.objectStore('newTransaction')
